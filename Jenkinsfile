  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sX GET http://archive.ubuntu.com/ubuntu/dists/bionic-updates/universe/binary-amd64/Packages.gz | gunzip |grep -A 7 -m 1 'Package: boinc-client' | awk -F ': ' '/Version/{print $2;exit}' ''',
            returnStdout: true).trim()
